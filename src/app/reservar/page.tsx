...
interface PackageItem { id: string; slug: string; name: string; description: string; public_price: number; includes: string[]; default_open_time: string; default_close_time: string; is_active: boolean }

type Step = 'experience' | 'date' | 'people' | 'extras' | 'ticket' | 'done'
...
export default function ReservarPage() {
  ...
  const basePrice = selectedPkg?.public_price || 0
  const eventPrice = dayEvent?.price_override ?? null
  const price = eventPrice ?? basePrice
  ...
  useEffect(() => {
    ...
    supabase.from('packages').select('*').eq('is_active', true).order('sort_order').then(({ data }) => { if (data) setPackages(data) })
  ...
  // FIX: handle adults/children as string to allow clearing
  const handleAdultsChange = (val: string) => {
    setAdultsStr(val)
    const n = parseInt(val)
    if (!isNaN(n) && n >= 1) setAdults(n)
  }
  const handleAdultsBlur = () => {
    const n = parseInt(adultsStr)
    if (isNaN(n) || n < 1) { setAdults(1); setAdultsStr('1') }
    else { setAdults(n); setAdultsStr(String(n)) }
  }
  const handleChildrenChange = (val: string) => {
    setChildrenStr(val)
    const n = parseInt(val)
    if (!isNaN(n) && n >= 0) setChildren(n)
  }
  const handleChildrenBlur = () => {
    const n = parseInt(childrenStr)
    if (isNaN(n) || n < 0) { setChildren(0); setChildrenStr('0') }
    else { setChildren(n); setChildrenStr(String(n)) }
  }

  // Sync string state when mode changes
  useEffect(() => { setAdultsStr(String(adults)); setChildrenStr(String(children)) }, [adults, children])

  const PriceSummary = () => (
    <div className="bg-black/30 border border-[#1e3838] p-3 mb-4 space-y-1 text-xs">
      <div className="flex justify-between"><span className="text-white/40">Paquete base ({personCount} × {fmtMoney(price)})</span><span className="text-[#f5f0e0]">{fmtMoney(subtotal)}</span></div>
        ...
        {specialEvents.filter(e => e.date >= today).sort((a, b) => a.date.localeCompare(b.date)).map(ev => (
          <div key={ev.id} onClick={() => selectEventDate(ev.date)} className="border-b border-[#1e3838] py-3 last:border-0 cursor-pointer hover:bg-[#c5a55a]/5 transition-all px-2 -mx-2">
            <div className="flex justify-between items-start">
              ...
              <span className="text-[.55rem] uppercase tracking-wider text-white/20">{ev.experience === 'both' ? 'Día y Noche' : ev.experience}</span>
            </div>
  ...
  return (
    <div className="min-h-screen bg-[#0c1a1a] flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-lg bg-[#132828] border border-[#1e3838] p-6 md:p-8">
        {/* Header with back link always visible */}
        <div className="flex items-center justify-between mb-4">
          <a href="/" className="text-xs text-white/30 hover:text-[#dcc07a]">← Volver al sitio</a>
        ...
        {step === 'experience' && (<div>
          ...
          <p className="text-sm text-white/50 mb-6">Elegí entre nuestras experiencias.</p>
          <div className="space-y-3">
            {packages.map(pkg => (
              <button key={pkg.slug} onClick={() => { setExp(pkg.slug); setStep('date') }} className="w-full text-left p-4 border border-[#1e3838] hover:border-[#c5a55a] transition-all group">
                <div className="flex justify-between items-center">
                  <div>
                    <div className="font-display text-lg font-medium text-[#f5f0e0] group-hover:text-[#dcc07a]">{pkg.name}</div>
                    <div className="text-xs text-[#c5a55a] italic">{pkg.description} · {pkg.default_open_time} a {pkg.default_close_time}</div>
                  </div>
                  <div className="font-display text-xl font-semibold text-[#dcc07a]">{fmtMoney(pkg.public_price)}</div>
                </div>
              </button>
            ))}
          </div>
        ...
        {step === 'date' && (<div>
          ...
          <p className="text-sm text-white/50 mb-1">Fechas disponibles para {selectedPkg?.name || exp}.</p>
          ...
          <div className="border border-[#1e3838] p-4 mb-4">
            ...
            <div className="grid grid-cols-7 gap-1">
              ...
              {Array(daysInMonth).fill(null).map((_, i) => { const day = i + 1, ds = getDS(day), av = isAvail(day), sel = ds === selectedDate, sp = isSpecial(day); return (<button key={day} disabled={!av} onClick={() => setSelectedDate(ds)} className={'p-2 text-xs rounded transition-all ' + (sel ? 'bg-[#c5a55a] text-[#0c1a1a] font-bold' : sp && av ? 'text-pink-400 bg-pink-400/10 hover:bg-pink-400/20 font-medium' : av ? 'text-[#f5f0e0] hover:bg-[#c5a55a]/10' : 'text-white/15 cursor-not-allowed')}>{day}</button>) })}
            </div>
          </div>
        ...
        {step === 'people' && (<div>
          ...
          <div className="space-y-2 mb-4">
            {[{ v: 'solo' as const, l: 'Solo/a', d: '1 persona' }, { v: 'acompanado' as const, l: 'Acompañado/a', d: '2 personas' }, { v: 'grupo' as const, l: 'En grupo', d: '3+ personas' }].map(o => (
              <label key={o.v} className={'flex items-center gap-3 p-3 border cursor-pointer transition-all ' + (mode === o.v ? 'border-[#c5a55a] bg-[#c5a55a]/5' : 'border-[#1e3838]')}
                onClick={() => { setMode(o.v); if (o.v === 'solo') { setAdults(1); setAdultsStr('1') } if (o.v === 'acompanado') { setAdults(2); setAdultsStr('2') } if (o.v === 'grupo' && adults < 3) { setAdults(3); setAdultsStr('3') } setError('') }}>
          ...
          {mode === 'grupo' && (<div className="grid grid-cols-2 gap-3 mb-4">
            <div><label className="block text-[.6rem] tracking-widest uppercase text-white/30 mb-1">Adultos</label>
              <input type="text" inputMode="numeric" value={adultsStr} onChange={e => handleAdultsChange(e.target.value.replace(/[^0-9]/g, ''))} onBlur={handleAdultsBlur} className="w-full p-2 bg-[#0f2020] border border-[#1e3838] text-[#f5f0e0] text-sm outline-none focus:border-[#c5a55a]" />
            </div>
            <div><label className="block text-[.6rem] tracking-widest uppercase text-white/30 mb-1">Niños (5-11)</label>
              <input type="text" inputMode="numeric" value={childrenStr} onChange={e => handleChildrenChange(e.target.value.replace(/[^0-9]/g, ''))} onBlur={handleChildrenBlur} className="w-full p-2 bg-[#0f2020] border border-[#1e3838] text-[#f5f0e0] text-sm outline-none focus:border-[#c5a55a]" />
            </div>
          </div>)}
          {error && <div className="bg-red-500/10 border border-red-500/30 text-red-400 text-xs p-3 mb-4">{error}</div>}
          <div className="flex gap-2">
            <button onClick={() => setStep('date')} className="flex-1 p-3 border border-[#1e3838] text-white/50 text-sm">← Volver</button>
        ...
        {step === 'extras' && (<div>
          ...
          {/* FIX: use onChange on checkbox, whole row clickable via label */}
          <label className="flex items-center gap-3 p-3 border border-[#c5a55a]/20 bg-[#c5a55a]/5 cursor-pointer mb-1">
            <input type="checkbox" checked={allInclusive} onChange={toggleAllInclusive} className="accent-[#c5a55a] w-4 h-4" />
          ...
          <div className="space-y-1 mb-4">
            {extrasList.map(ext => (
              <label key={ext.key} className={'flex items-center gap-3 p-2.5 cursor-pointer transition-all border ' + (selectedExtras.includes(ext.key) ? 'border-[#c5a55a]/20 bg-[#c5a55a]/5' : 'border-transparent hover:bg-white/[.02]')}>
                <input type="checkbox" checked={selectedExtras.includes(ext.key)} onChange={() => toggleExtra(ext.key)} className="accent-[#c5a55a] w-3.5 h-3.5" />
        ...
        {step === 'ticket' && (<div>
          ...
          <div className="space-y-3 mb-4">
            <input placeholder="Nombre completo" value={name} onChange={e => setName(e.target.value)} className="w-full p-2.5 bg-[#0f2020] border border-[#1e3838] text-[#f5f0e0] text-sm outline-none focus:border-[#c5a55a] placeholder:text-white/20" />
            <input placeholder="WhatsApp (+54 9 11...)" value={phone} onChange={e => setPhone(e.target.value)} className="w-full p-2.5 bg-[#0f2020] border border-[#1e3838] text-[#f5f0e0] text-sm outline-none focus:border-[#c5a55a] placeholder:text-white/20" />
            <input placeholder="Email (opcional)" value={email} onChange={e => setEmail(e.target.value)} className="w-full p-2.5 bg-[#0f2020] border border-[#1e3838] text-[#f5f0e0] text-sm outline-none focus:border-[#c5a55a] placeholder:text-white/20" />
            <input placeholder="Código de cupón (opcional)" value={coupon} onChange={e => setCoupon(e.target.value)} className="w-full p-2.5 bg-[#0f2020] border border-[#1e3838] text-[#f5f0e0] text-sm outline-none focus:border-[#c5a55a] placeholder:text-white/20" />
          </div>
          <div className="bg-black/20 border border-[#1e3838] p-4 text-xs space-y-1 mb-4">
            <div className="flex justify-between"><strong className="text-[#f5f0e0]">{selectedPkg?.name || exp}</strong><span className="text-white/40">{personCount} persona{personCount > 1 ? 's' : ''}</span></div>
            ...
            <div className="text-[.6rem] uppercase tracking-wider text-white/20 mb-1">El paquete base incluye:</div>
